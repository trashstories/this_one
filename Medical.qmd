...
colors3 <- c("#0ACCC5", "#CCAC14", "#3ACC14", 
             "#CC1F14", "#CC0AA4", "#4F008C", "#7400CC", 
             "#1471CC", "#805713", "#B785DD")


# colors <- c("#CC0AA4", "#7400CC", "#0E0ACC", "#0ACCC5", "#3ACC14")
...
sym <- sym %>% 
  ...
  mutate(state = case_when(date == "2024-09-04" ~ "PA",
                           date > "2024-09-04" & date <= "2024-09-08" ~ "WV",
                           date == "2024-09-09" ~ "NC",
                           date > "2024-09-09" & date <= "2024-09-12" ~ "SC",
                           date > "2024-09-12" ~ "GA",
                           TRUE ~ state))
...
pain1 <- sym %>% 
  pivot_longer(c(overall, forearm, back, fatigue, dizziness, headache)) 
...
sym %>%
  ...
  ggplot(aes(date, cyst,
             color = activeCyst,
             fill = activeCyst)) +
  geom_vline(xintercept = c(as.Date("2024-07-17"), # doxy start
                            as.Date("2024-08-04"), # doxy end
                            as.Date("2024-09-08") # doxy start
                            ),
             linewidth = .75,
  ...
  geom_curve(x = as.Date("2024-07-08"), 
             ...
             color = "#310873") +
  annotate("text", 
           x = as.Date("2024-08-28") - .5,
           y = 2,
           label = "Doxy\nStarted",
           family = "Changa",
           color = "#310873") +
  geom_curve(x = as.Date("2024-08-28"), 
             xend = as.Date("2024-09-07"), 
             y = 2.2, yend = 2.55,
             curvature = -0.25,
             arrow = arrow(angle = 25,
                           length = unit(0.75, "lines")),
...
poop %>% 
  ...
  annotate("text", 
           ...
           color = "#310873",
           angle = 90) +
  scale_x_date(breaks = date_breaks(width = "4 weeks"),
...
mids_before <- sts_k %>% 
  ...
  mutate(mood_mid = mean_mood / 2,
         angst_mid = mean_angst /2) %>% 
  select(state, mood_mid, angst_mid)
...
sts_k %>% 
  ...
  scale_y_continuous(breaks = seq(0,10,2), 
                     limits = c(0,10)) +
  scale_fill_steps2(low = "white",
                    mid = lighten("#0E0ACC", .35),
                    high = "black",
                    midpoint = mean(sts_k$n),
                    space = "Lab",
                    na.value = "grey50",
                    breaks = c(7,30,60,90,120)) +
  scale_color_manual(values = c("white", "white"), guide = "none") +
  annotate("text",
           x = c(8.86,14,15),
           y = c(4.45,mids_before$mood_mid[1],mids_before$mood_mid[2]),
  ...
  labs(x = NULL,
       ...
       fill = str_wrap("Number of Days", 8),
       color = NULL,
...
sts_k %>% 
  ggplot(aes(reorder(state, mean_angst), mean_angst, 
             fill = n, color = after_k)) +
  geom_col(position = position_dodge(width = .5)) +
  scale_y_continuous(breaks = seq(0,10,2), 
                     limits = c(0,10)) +
  scale_fill_steps2(low = "white",
                    mid = lighten("#0E0ACC", .25),
                    high = "black",
                    midpoint = mean(sts_k$n),
                    space = "Lab",
                    na.value = "grey50",) +
  ...
  annotate("text",
           x = c(8.86,13,15),
           y = c(4.9,mids_before$angst_mid[1],mids_before$angst_mid[2]),
           label = "Before K Treatment",
           angle = 90,
           family = "Cambria",
           color = "white",
...
k_si_night <- k_si_night_ %>% 
  mutate(rate = ifelse(time == "Before", (value / (before)) * 100,
                       (value / (since)) * 100))
