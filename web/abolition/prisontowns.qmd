...
prisons %>% 
  ...
  labs(x = "Year of Construction",
       y = NULL,
       fill = NULL,
       title = "Construction Timeline of Prisons in Georgia") +
  theme_light() +
...
cap_prisons %>% 
  gt() %>% 
  tab_header(md("**Average Capacity and Type of Prison**")) %>% 
  cols_label(Facility_Type = md("*Type of Facility*"),
             mean_cap = md("*Average Capacity*")) %>% 
  fmt_number(columns = mean_cap,
             decimals = 0) %>% 
...
knitr::include_graphics("figures/overviewmap.png")
...
```{r counties}
#| label: tbl-counties
#| tbl-cap: "Summary of Georgia Counties" 
#| tbl-align: 'center'
...
countsum %>% 
  gt() %>% 
  tab_header(md("**Summary of Georgia Counties**")) %>% 
  cols_label(x = md(""),
             y = md("")) %>% 
  fmt_number(columns = y,
             decimals = 0) %>% 
  cols_align(align = "center",
             columns = y)
...
knitr::include_graphics("figures/assaultmap.png")
...
knitr::include_graphics("figures/incomemap.png")
...
knitr::include_graphics("figures/unempmap.png")
...
knitr::include_graphics("figures/povertymap.png")
...
knitr::include_graphics("figures/attendmap.png")
...
focus <- counties2 %>% 
  filter(Facility_Name == "RIVERBEND CORR FACILITY" | 
           Facility_Name == "JENKINS CORR FACILITY") %>% 
...
focus %>%
  gt() %>% 
  tab_header(md("**Differences for Baldwin/Jenkins Counties and Rural Averages**")) %>% 
  fmt_number(columns = !c(County, Income),
             decimals = 2,
             force_sign = TRUE) %>% 
  fmt_number(columns = Income,
             decimals = 0,
             force_sign = TRUE) %>% 
  cols_align(align = "center",
             columns = !County) %>% 
  cols_width(Assault ~ px(100),
             Income ~ px(100),
             Unemployment ~ px(100),
             Poverty ~ px(100),
             Attendance ~ px(100))
...
[**Georgia Counties**](https://gsu.maps.arcgis.com/home/item.html?id=dc20713282734a73abe990995de40497)
