...
ALAR <- deaths %>% 
  ...
  mutate(after_change = ifelse(year >= 1975, TRUE, FALSE)) %>% 
  mutate(is_alabama = ifelse(state_name == "Alabama", TRUE, FALSE)) %>% 
  filter(year <= 1983,
         agegr == "18-20 yrs",
         dtype == "all")

ALAK <- deaths %>% 
  filter(state_name %in% c("Alabama", "Alaska")) %>% 
  mutate(after_change = ifelse(year >= 1975, TRUE, FALSE)) %>% 
  mutate(is_alabama = ifelse(state_name == "Alabama", TRUE, FALSE)) %>% 
  filter(year <= 1983,
         agegr == "18-20 yrs",
...
model <- lm(mrate ~ is_alabama + after_change + is_alabama * after_change,
            data = ALAR)

tidy(model)

model1 <- lm(mrate ~ is_alabama + after_change + is_alabama * after_change,
            data = ALAK)
